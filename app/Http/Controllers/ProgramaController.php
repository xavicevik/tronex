...
class ProgramaController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (is_null($filtros)){
            $programas = Programa::orderBy($sortBy, $sortOrder)
                                ->with('usuario')
                                ->with('kamara')
                                ->with('creador')
            ...
            if(!is_null($filtros->fechainicio) && $filtros->fechainicio <> '') {
                $programas = $programas->where('programas.created_at', '>=', $filtros->fechainicio);
            }
            if(!is_null($filtros->fechafin) && $filtros->fechafin <> '') {
                $programas = $programas->where('programas.created_at', '<=', $filtros->fechafin);
            }
            /*
            if(!is_null($filtros->cliente) && $filtros->cliente <> '') {
                $programas = $programas->join('users as t1', 'dispositivos.idcliente', '=', 't1.id')
                    ->where('t1.nombre', 'like', '%'.$filtros->cliente.'%')
                    ->orWhere('t1.apellido', 'like', '%'.$filtros->cliente.'%');
            }
            if(!is_null($filtros->vendedor) && $filtros->vendedor <> '') {
                $programas = $programas->join('users as t2', 'dispositivos.idvendedor', '=', 't2.id')
                    ->where('t2.nombre', 'like', '%'.$filtros->vendedor.'%')
                    ->orWhere('t2.apellido', 'like', '%'.$filtros->vendedor.'%');
            }

            if(!is_null($filtros->venta) && $filtros->venta <> '') {
                $programas = $programas->where('dispositivos.id', 'like', '%'.$filtros->venta.'%');
            }
            if(!is_null($filtros->puntoventa) && $filtros->puntoventa <> '') {
                $programas = $programas->join('puntos_dispositivos', 'dispositivos.idpuntoventa', '=', 'puntos_dispositivos.id')
                    ->where('puntos_dispositivos.nombre', 'like', '%'.$filtros->puntoventa.'%');
            }

            if(!is_null($filtros->nombre)) {
                $programas = $programas->where('nombre', 'like', '%'.$filtros->nombre.'%');
            }
            */

            $programas = $programas->select('programas.*')->paginate(self::canPorPagina);
        }

        if ($request->has('ispage') && $request->ispage){
            return ['data' => $programas];
        } else {
            return Inertia::render('Programas/Index', ['data' => $programas]);
        }
    }
